...
var AudioHandler = function() {
  ...
  var levelsData2 = []; //levels of each frequecy - from 0 - 1 . no sound is 0. Array [levelsCount]
  ...
  function update(){
    ...
    //normalize waveform data
    var fqBinSize = Math.floor(binCount / fqRows);
    for(var i = 0; i < fqRows; i++) {
      var sum = 0;
      for(var j = 0; j < fqBinSize; j++) {
        sum += timeByteData[(i * fqBinSize) + j];
      }
      waveDataRedux[i] = ((sum - (128 * fqBinSize))  /(128 * fqBinSize) )* ControlsHandler.audioParams.volSens;
     // waveDataRedux[i] = ((timeByteData[i] - 128) /128 )* ControlsHandler.audioParams.volSens;
    }
    fqBufferArr.unshift(waveDataRedux.slice());
    if(fqBufferArr.length > 128){
      fqBufferArr.pop();
    }
    ...
    for(var i = 0; i < eqRows; i++) {
      var sum = 0;
      for(var j = 0; j < levelBins; j++) {
        sum += freqByteData[(i * levelBins) + j];
      }
      levelsData2[i] = freqByteData[i]/256;//sum / levelBins/256 * ControlsHandler.audioParams.volSens; //freqData maxs at 256
    ...
    eqBufferArr.unshift(levelsData2.slice());
    if(eqBufferArr.length > 128){
      eqBufferArr.pop();
    }
    ...
    //scale = 1.2 + ((6 - Math.min(beatTime, 6)) * .02);
    scale = 1.2 + (level * .1);
    scale += ((6 - Math.min(beatTime, 6)) * .002);
